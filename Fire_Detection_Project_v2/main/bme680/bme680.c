...
#include "bme680/bme680_reg_def.h"
...
static int16_t bme680_convert_temperature(bme680_t *dev, uint32_t raw_temperature)
{
	...
	var2 = (((((raw_temperature >> 4) - ((int32_t)cd->par_t1)) * ((raw_temperature >> 4) - ((int32_t)cd->par_t1))) >> 12) * ((int32_t)cd->par_t3)) >> 14;
...
static uint32_t bme680_convert_humidity(bme680_t *dev, uint16_t raw_humidity)
{
	...
	var1 = (int32_t)(raw_humidity - ((int32_t)((int32_t)cd->par_h1 << 4))) - (((temp_scaled * (int32_t)cd->par_h3) / ((int32_t)100)) >> 1);
	var2 = ((int32_t)cd->par_h2 * (((temp_scaled * (int32_t)cd->par_h4) / ((int32_t)100)) + (((temp_scaled * ((temp_scaled * (int32_t)cd->par_h5) / ((int32_t)100))) >> 6) / ((int32_t)100)) + (int32_t)(1 << 14))) >> 10;
	var3 = var1 * var2;
...
static uint8_t bme680_heater_resistance(const bme680_t *dev, uint16_t temp)
{
	...
	res_heat_x = (uint8_t)(3.4 * ((var5 * (4.0 / (4.0 + (double)cd->res_heat_range)) * (1.0 / (1.0 + ((double)cd->res_heat_val * 0.002)))) - 25));
	return res_heat_x;
}
...
esp_err_t bme680_get_measurement_duration(const bme680_t *dev, uint32_t *duration)
{
	...
	*duration += 1250;
	...
	if (dev->settings.heater_profile != BME680_HEATER_NOT_USED && dev->settings.heater_duration[dev->settings.heater_profile] && dev->settings.heater_temperature[dev->settings.heater_profile])
	{
		// gas heating time
...
esp_err_t bme680_get_results_float(bme680_t *dev, bme680_values_float_t *results)
{
	...
	results->gas_resistance = fixed.gas_resistance / 1000.0f;
...
esp_err_t bme680_set_heater_profile(bme680_t *dev, uint8_t profile, uint16_t temperature, uint16_t duration)
{
	...
	ESP_LOGD(TAG, "Setting heater profile %d done: temperature=%d duration=%d"
					 ...
					 profile, dev->settings.heater_temperature[profile],
